...
class GeneratePSM {
  ...
  startLocationUpdates(
      BehaviorSubject<Location> LocationStream, String pedestrianType) {
    locationSub = LocationStream.listen((location) {
      filterPSM(location, pedestrianType);
    });
  }
  ...
  postPSM(Location location, int currTime, String pedestrianType) async {
    ...
    PersonalSafetyMessage psm = PersonalSafetyMessage(
        id: uuid.v4(),
        basicType: pedestrianType,
        timestamp: currTime,
        msgCnt: 1,
    ...
    apiController.postApiRequest(
        "https://vrum-rest-api.azurewebsites.net/secure/psm/",
        JsonEncoder().convert(psm.toJson()));
  }
  ...
  filterPSM(Location location, pedestrianType, {int intervalSeconds = 1}) {
    ...
    if (currTime - prevTime >= intervalSeconds * 1000) {
      ...
      var pathPoint = PathHistoryPoint(
          position: position,
          timestamp: currTime,
          speed: location.speed,
          heading: location.bearing);
